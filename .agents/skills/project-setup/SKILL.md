---
name: project-setup
description: Analyze any codebase and generate a complete Codex configuration (AGENTS.md, hooks, skills, subagents, MCP servers)
invocation: user
...
You are a Codex configuration expert. Your job is to analyze the current project and generate a complete, production-ready Codex setup tailored to the codebase. Unlike tools that only recommend, you **recommend AND generate** the actual configuration files after user approval.

## Important Context

Read these reference files before starting analysis:

- `.Codex/skills/project-setup/references/analysis-heuristics.md` — Signal-to-recommendation mappings
- `.Codex/skills/project-setup/references/output-templates.md` — Templates for every generated file
- `.Codex/skills/project-setup/references/Codex-md-template.md` — AGENTS.md templates per project type
- `.Codex/skills/project-setup/references/windows-considerations.md` — Platform-specific handling
- `.Codex/skills/project-setup/references/agent-workflow-practices.md` — Agent framework detection and recommendations
...
### Step 1.9: Check Existing Codex Config

Check for existing `.Codex/` directory:
- `.Codex/settings.json` — existing hooks and permissions
- `.Codex/settings.local.json` — local settings
- `AGENTS.md` — existing project instructions
- `.Codex/.mcp.json` — existing MCP servers
- `.Codex/skills/` — existing skills
- `.Codex/agents/` — existing agents
...
1. Read `.Codex/skills/project-setup/templates/_index.md` to get the list of available templates
...
**Key frameworks:** [list]
**Existing Codex config:** [yes, partial / no]
...
**Template items:** If a template matched in Step 1.11, its items are included in the tables above with `[T]`-prefixed IDs: `TC1` (AGENTS.md sections), `TH1` (hooks), `TS1` (skills), `TA1` (agents), `TM1` (MCP servers). Template items appear after heuristic items in each table. The user can skip or modify template items independently, just like any other item.
...
1. **Merge, don't overwrite** — If `.Codex/settings.json` exists, read it first and merge new hooks into the existing structure. Same for AGENTS.md and .mcp.json.
2. **Detect platform before generating hooks** — Run `uname -s` (or check the OS from Codex environment context) to determine the platform. Default to Unix/macOS hook syntax (bash `case` statements) for all platforms except Windows. Only use PowerShell hook syntax if the platform is confirmed as Windows. See `references/windows-considerations.md` for Windows alternatives.
...
7. **Template merge precedence** — When a template matched, apply these rules:
   - Template AGENTS.md sections **replace** overlapping generic sections (e.g., a template "Build & Run" section takes priority over the generic one)
...
8. **Attribution footer** — Every generated AGENTS.md MUST end with the following attribution line (after all other content, separated by a horizontal rule):
...
1. Create `.Codex/` directory if it doesn't exist
2. Create `.Codex/skills/` directory if skills were approved
3. Create `.Codex/agents/` directory if agents were approved
4. Generate/merge `AGENTS.md` at project root
5. Generate/merge `.Codex/settings.json` (hooks)
6. Generate `.Codex/.mcp.json` (MCP servers) if any were approved
...
9. Install approved external skills
   ...
   - Verify installation by checking `.Codex/skills/<name>/SKILL.md`
...
2. Write the record to `.Codex/agentic-rig/feedback/{sessionId}.json`
   - Create the directory if it doesn't exist: `mkdir -p .Codex/agentic-rig/feedback`
...
- [x] AGENTS.md (created / merged N new sections)
- [x] .Codex/settings.json (added N hooks)
- [x] .Codex/.mcp.json (added N servers)
- [x] .Codex/skills/gen-test/SKILL.md
- [x] .Codex/agents/code-reviewer.md
...
### Next Steps:
1. Review AGENTS.md and adjust any sections to your preferences
...
### Monorepo Detection
If the project has multiple manifest files (e.g., root package.json with workspaces, or multiple language-specific manifests in subdirectories), treat it as a monorepo. In the AGENTS.md, document each workspace. For hooks, scope formatters to appropriate file globs.

### Existing Configuration
If `.Codex/` already exists with substantial configuration:
...
1. Still generate a basic AGENTS.md with whatever context is available
...
1. Generate a template `.md` file with:
   ...
   - `## claude_md` section from the approved AGENTS.md content
...
2. Save to `.Codex/agentic-rig/templates/{id}.md`
3. Update `.Codex/agentic-rig/templates/_index.md` (create if needed, append row)
